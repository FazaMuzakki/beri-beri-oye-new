/* ===========================================================
   Penilaian Kabupaten/Kota – Replikasi Formulir Kondisi (tanpa Excel)
    ...
    // ================== C1 ==================
{
    ...
    },
    {
      no: 'C4-table',
      type: 'table-input',
      label: 'Kapasitas dan nama bank sampah di tiap RW ',
      required: true,
      rows: 4,
      hasLeftLabels: true,
      rowLabels: [
...

{
...
},
  {
    ...
    },
    {
      no: 'C7-table',
      type: 'table-input',
      label: 'inovasi pengelolaan sampah 3R di tiap RW',
      required: true,
      rows: 5,
      hasLeftLabels: true,
      showOn: ['b', 'c', 'd'],
...

{
  ...
  items: [
   {
    ...
    },
    {
      no: 'F1-table',
      type: 'table-input',
      label: 'Upaya peresapan dan pemanenan air hujan di tiap RW',
  ...
  // Ekspose konfigurasi ke global agar dapat dipakai di modul ekspor PDF
  try { window.FORM_CONFIG_KABKOTA = FORM_CONFIG; } catch (e) { /* ignore */ }
  ...
  // ==============  B. RENDER & LOGIC  =================
  var form = document.getElementById('formKabkota');
  var dotsWrap = document.getElementById('kabkotaStepDots'); 
  var totalSkorEl = document.getElementById('kabkotaTotalSkor');
  var sectionSkorEl = document.getElementById('kabkotaSectionSkor');
  var kategoriOutEl = document.getElementById('kabkotaKategori');
...
function recalcAll() {
    ...
    // Component score elements (kabkota namespace)
    var componentScores = {
      'A': document.getElementById('kabkotaComponentA'),
      'B': document.getElementById('kabkotaComponentB'),
      'C': document.getElementById('kabkotaComponentC'),
      'D': document.getElementById('kabkotaComponentD'),
      'E': document.getElementById('kabkotaComponentE'),
      'F': document.getElementById('kabkotaComponentF')
    };
...
function updateScoreDisplay(total) {
    ...
    try {
        ...
        var selectedAssessment = JSON.parse(localStorage.getItem('kabkota_selected_assessment') || '{}');
        var menujuKategori = selectedAssessment.menuju_kategori || '';
        
        // Calculate progress percentage
        var progressPercentage = (ANSWERED_QUESTIONS / TOTAL_QUESTIONS) * 100;
        
    // Update progress bar if exists (kabkota IDs)
    var progressBar = document.getElementById('kabkotaProgressBar');
    var progressText = document.getElementById('kabkotaProgressText');
    if (progressBar) {
...
function setupCheckboxRequired(){
  ...
  initProgress();
  render();
// Pastikan tetap diekspor bila terjadi reload hot atau urutan script berbeda
try { if (!window.FORM_CONFIG_KABKOTA) window.FORM_CONFIG_KABKOTA = FORM_CONFIG; } catch (e) { /* ignore */ }
})();
